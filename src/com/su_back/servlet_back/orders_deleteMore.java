...
/**
 * Servlet implementation class orders_deleteMore
 */
@WebServlet("/orders_deleteMore.do")
public class orders_deleteMore extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public orders_deleteMore() {
        super();
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		log.setDs("在"+sdf.format(date)+"进行了批量删除订单操作");
		log.setName(name);
		log.setType("删除");
		...
		orders_service os=new orders_service();	
			try {
				...
				for(int i=0;i<id.length;i++){
					os.delete_orders(Integer.parseInt(id[i]));//根据id删除
				}
				os.log(log);
				
				response.sendRedirect("Back/O_orders_delMore.jsp");
			} catch (Exception e) {
