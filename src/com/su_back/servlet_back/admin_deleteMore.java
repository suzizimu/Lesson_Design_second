...
/**
 * Servlet implementation class admin_deleteMore
 */
@WebServlet("/admin_deleteMore.do")
public class admin_deleteMore extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public admin_deleteMore() {
        super();
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		log.setDs("在"+sdf.format(date)+"进行了批量删除管理员操作");
		log.setName(name);
		log.setType("删除");
		...
		admin_service as=new admin_service();	
			try {
				...
				for(int i=0;i<id.length;i++){
					as.delete_admin(Integer.parseInt(id[i]));//根据id删除
				}
				as.log(log);
				
				response.sendRedirect("Back/A_admin_delMore.jsp");
			} catch (Exception e) {
