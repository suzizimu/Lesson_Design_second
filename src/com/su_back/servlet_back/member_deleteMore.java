...
/**
 * Servlet implementation class member_deleteMore
 */
@WebServlet("/member_deleteMore.do")
public class member_deleteMore extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public member_deleteMore() {
        super();
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		log.setDs("在"+sdf.format(date)+"进行了批量删除用户操作");
		log.setName(name1);
		log.setType("删除");
		...
		member_service ms=new member_service();	
			try {
				...
				for(int i=0;i<id.length;i++){
					ms.delete_member(Integer.parseInt(id[i]));//根据id删除
				}
				ms.log(log);
				
				response.sendRedirect("Back/M_member_delMore.jsp");
			} catch (Exception e) {
