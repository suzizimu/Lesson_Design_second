	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			<tr class="text-c">
				...
				<th width="250">商品名</th>										
				<th width="250">卖家 </th>				
				<th width="250">我的评论</th>
				<th width="250">评论日期</th>
				<th width="250">操作</th>
			</tr>		                               
		</thead>
		<tbody>
			<c:forEach items="${list}" var="buyer_reply">
		     <tr class="text-c">
		       <td><input type="checkbox" value="${buyer_reply.ID}" name="check"></td>
		       <td>${buyer_reply.ID}</td>
		       <td>${buyer_reply.orderNum}</td>
		       <td>${buyer_reply.goodsID}</td>
		       <td>${buyer_reply.goodsName}</td>	
		       <td>${buyer_reply.seller}</td>		     
		       <td>${buyer_reply.introduce}</td>
		       <td>${buyer_reply.date}</td>		 
			   <td class="td-manage">			      			   
				  <a title="删除" href="javascript:;" onclick="buyer_reply_del(${buyer_reply.ID})" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
			   </td>
			</tr>
		  </c:forEach>		
		</tbody>
