...
<div >
	<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 收藏管理 <span class="c-gray en">&gt;</span> 产品列表 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
	<div class="pd-20">
			...
			<table class="table table-border table-bordered table-bg table-hover table-sort">
				...
				<tbody>
			       <c:forEach items="${list}" var="collect">
		             <tr class="text-c">
		                <td><input type="checkbox" value="${collect.ID}" name="check"></td>
		                <td>${collect.ID}</td>
		                <td><img src="${collect.url}"></td>
		                <td>${collect.goodsID}</td>
		                <td>${collect.name}</td>		               
		                <td>${collect.priceNow}</td>
		                <td>${collect.goodsType}</td>		               	               		                					
						<td class="td-manage">			 
					       <a style="text-decoration:none" class="ml-5" onClick="goods_del(${collect.ID})" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
						</td>
					</tr>
				  </c:forEach>
				</tbody>
	...
	layer.confirm('确认要删除吗？',function(){
		$.post("/Lesson_Design_second/goods_delete.do",{goods_id:id},function(data){
			if(data.info=="删除成功"){
				...
				window.location.href="/Lesson_Design_second/goods_list_front.so";					
			}else{
				layer.msg('删除失败！');
			}			
		},"json")
