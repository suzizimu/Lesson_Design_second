...
<div >
	<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>买入的商品<span class="c-gray en">&gt;</span> 买入列表 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
	<div class="pd-20">
				...
				<tbody>
			       <c:forEach items="${list}" var="orders">
		             <tr class="text-c">
		                ...
		                <td>${orders.seller}</td>			
						<td class="td-manage">	
						   <a title="评论" href="javascript:;" onclick="orders_reply(${orders.ID})" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>					   
...
///************************************************************
/*反馈-回复*/
function orders_reply(id){
	layer.open({
		type:2
		,area:['900px','550px']
	    ,title:'买家评论'
	    ,maxmin:true
	    ,content:'../buyer_reply_add.so?id='+id    
	});
}
	...
	layer.confirm('确认要退货吗？',function(){
		$.post("/Lesson_Design_second/orders_delete.do",{orders_id:id},function(data){
			if(data.info=="退货成功"){
				...
				window.location.href="/Lesson_Design_second/orders_list_front.so";					
			}else{
				layer.msg('退货失败！');
			}			
		},"json")
