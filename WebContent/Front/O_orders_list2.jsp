...
<div >
	<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>卖出的东西<span class="c-gray en">&gt;</span>卖出列表 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
	<div class="pd-20">
				...
				<tbody>
			       <c:forEach items="${list}" var="orders">
		             <tr class="text-c">
		                ...
		                <td>${orders.seller}</td>			
						<td class="td-manage">				 				   
						   <a style="text-decoration:none" class="ml-5" onClick="orders_del(${orders.ID})" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
						</td>
					</tr>
				  </c:forEach>
				</tbody>
	...
	layer.confirm('确认要退货吗？',function(){
		$.post("/Lesson_Design_second/orders_delete.do",{orders_id:id},function(data){
			if(data.info=="退货成功"){
				...
				window.location.href="/Lesson_Design_second/orders_list_front2.so";					
			}else{
				layer.msg('退货失败！');
			}			
		},"json")
