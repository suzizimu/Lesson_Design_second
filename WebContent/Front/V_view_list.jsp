...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>浏览记录 <span class="c-gray en">&gt;</span>列表<a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<c:forEach items="${list}" var="view">
		     <tr class="text-c">
		       <td><input type="checkbox" value="${view.ID}" name="check"></td>	
		       <td>${view.ID}</td>	       
		       <td><img src="${view.url}"></td>
		       <td>${view.goodsID}</td>
		       <td>${view.goodsName}</td>	
		       <td>${view.goodsType}</td>		     
		       <td>${view.priceNow}</td>
		       <td>${view.date}</td>		 
			   <td class="td-manage">			      			   
				  <a title="删除" href="javascript:;" onclick="view_del(${view.ID})" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
			   </td>
			</tr>
		  </c:forEach>		
		</tbody>
...
/*举报-删除*/
function view_del(id){
	layer.confirm('确认要删除吗？',function(){
		$.post("/Lesson_Design_second/view_delete.so",{view_id:id},function(data){
			if(data.info=="删除成功"){
				layer.msg('已删除!',{icon:1,time:1000});
				window.location.href="/Lesson_Design_second/view_list.so";					
			}else{
				layer.msg('删除失败！');
			}			
		},"json")
